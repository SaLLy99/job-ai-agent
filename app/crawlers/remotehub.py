...
class RemoteHubCrawler:

    BASE_URL = "https://www.remotehub.com/jobs"
    ...
    def crawl(self, keywords=None):
        ...
        for a in soup.select("a[href*='/jobs/details/'], a[href*='/job/']"):
            href = a.get("href", "")
            title = a.text.strip()
            ...
            full_url = href if href.startswith("http") else f"https://www.remotehub.com{href}"
            ...
            jobs.append({
                "id": hashlib.md5(f"remotehub:{title}:{company}".encode()).hexdigest(),
                "title": title,
                "company": company,
                "url": full_url,
                "location": "Worldwide",
